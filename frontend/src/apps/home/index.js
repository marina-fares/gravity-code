...
export default function Home({shift}){
    ...
    return (
        <>
            { ShiftDetails && (ShiftDetails.current_shift_id !== null) && (ShiftDetails.start_time !== null) ? <Grid container spacing={2}>
                <Grid item xs={12} md={2} style={{marginTop:85}}>
                <HomeInput date={date} setDate={setDate} setProduct={setProduct}/>
                </Grid>
            
                <Grid item xs={12} md={10} style={{marginTop:30}}>
                    <h1>Available List</h1>
                    <AvailableSlots  date={date} product={product} />
                </Grid>
            </Grid>
            :
