		...
		<div ref={ref} style={{ width: '80mm' }}>
			...
			{inventory &&
				Object.keys(inventory).map((key, index) => {
					return ( inventory[key].sold + inventory[key].sold_at_square !==0 && <PaperRow right={key} left={`${inventory[key]?.sold + inventory[key]?.sold_at_square} /${inventory[key]?.start_shift}`} />)
				})}
		</div>
	);
});

const InvoiceFullShift = React.forwardRef(({ shift, cash, visa }, ref) => {
	...
	let total_in_out = shift?.shift_money_cash - shift?.refund_cash;
	let in_drawer_cash = shift?.start_shift_cash + total_in_out;
	let actual_in_drawer_cash = cash;
	let difference_cash = actual_in_drawer_cash  - in_drawer_cash ;
	let in_drawer_visa = shift?.shift_money_visa;
	let actual_in_drawer_visa = visa;
	let difference_visa =  actual_in_drawer_visa - in_drawer_visa + shift?.refund_visa;
	let user = get_user_and_jwt().user;
	console.log("Invoice");
	console.log(user)
	let inventory = shift?.inventory;
	return (
		<div ref={ref} style={{ width: '80mm' }}>
			<PageHeader title={`Drawer Report: ${shift.branch_name} - ${user.username} `} />
			<FullPageRow
			...
			<FullPageRow text='Summary' />
			<hr style={{ margin: '10px' }} />
			<PaperRow right='Starting Cash' left={`${shift?.start_shift_cash}`} />
			<PaperRow right='Cash Sales' left={`${shift?.shift_money_cash}`} />
...
export const InvoicePrint = ({ shift , sub_shift }) => {
	...
	useEffect(() => {
		shift.actual_cash = parseInt(cash)
		shift.actual_visa = parseInt(visa)
		set_shift_fun(shift)
		...
		<div>
			...
			<div className ="d-flex justify-content-center w-100">
				...
				<div  style={{ border: '1px solid #000', padding: '10px', margin: '10px' }}>	
					...
					<InvoiceFullShift ref={componentRef_full_shift} shift={shift} cash={cash} visa={visa} sub_shift={sub_shift} className ="d-flex justify-content-end"/>
					<ReactToPrint
						trigger={() => (
						...
						content={() => componentRef_full_shift.current}
					/>
				</div>
			</div>
		</div>
